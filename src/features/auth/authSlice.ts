...
import { loginLocal, logout, meLocal, meGoogle, meKakao } from '@/services/api/authApi'
...
export const hydrateAuthThunk = createAsyncThunk('auth/hydrate', async (_: void, { rejectWithValue }) => {
...
/** ✅ 로컬 로그인 */
export const loginThunk = createAsyncThunk('auth/login', async (payload: { idOrEmail: string; password: string }, { rejectWithValue }) => {
   try {
...
const slice = createSlice({
   ...
   reducers: {
      ...
      setUser(state, action: PayloadAction<User | null>) {
         ...
         state.provider = action.payload ? (action.payload.provider ?? 'local') : 'none'
      },
   },
   extraReducers: (b) => {
      // hydrate
