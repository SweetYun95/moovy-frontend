...
// Thunks
// ─────────────────────────────────────────────────────────────
export const getRepliesByComment = createAsyncThunk('replies/getByComment', async ({ commentId, page = 1, size = 20 }: { commentId: number; page?: number; size?: number }, { rejectWithValue }) => {
   try {
      // api.fetchReplies는 res.data를 반환하도록 구현되어 있음
      const data: { items: Reply[]; meta: RepliesMeta } = await api.fetchReplies(commentId, { page, size })
      return { commentId, ...data }
   } catch (err: any) {
      return rejectWithValue(err?.response?.data || { message: '대댓글 조회 실패' })
   }
})

export const createReplyThunk = createAsyncThunk('replies/create', async ({ comment_id, content }: { comment_id: number; content: string }, { rejectWithValue }) => {
   try {
      const data: Reply = await api.createReply({ comment_id, content })
      return data
   } catch (err: any) {
      return rejectWithValue(err?.response?.data || { message: '대댓글 작성 실패' })
   }
})

export const updateReplyThunk = createAsyncThunk('replies/update', async ({ reply_id, content }: { reply_id: number; content: string }, { rejectWithValue }) => {
   try {
      // 백엔드 응답: { message, reply }
      const data: { message: string; reply: Reply } = await api.updateReply(reply_id, { content })
      return data.reply
   } catch (err: any) {
      return rejectWithValue(err?.response?.data || { message: '대댓글 수정 실패' })
   }
})

export const deleteReplyThunk = createAsyncThunk('replies/delete', async ({ reply_id }: { reply_id: number }, { rejectWithValue }) => {
   try {
      await api.deleteReply(reply_id)
      return { reply_id }
   } catch (err: any) {
      return rejectWithValue(err?.response?.data || { message: '대댓글 삭제 실패' })
   }
})
...
const repliesSlice = createSlice({
   ...
   initialState,
   reducers: {
      clearReplies(state, action: PayloadAction<{ commentId?: number } | undefined>) {
   ...
   extraReducers: (builder) => {
      ...
      builder
         .addCase(getRepliesByComment.pending, (state, action) => {
            ...
            state.byCommentId[commentId] ??= { items: [], meta: null, loading: false, error: null }
         ...
         .addCase(getRepliesByComment.fulfilled, (state, action) => {
            ...
            state.byCommentId[commentId] = { items, meta, loading: false, error: null }
         })
         .addCase(getRepliesByComment.rejected, (state, action) => {
            ...
            state.byCommentId[commentId].error = (action.payload as any)?.message || '대댓글 조회 실패'
         })
      ...
      builder
         ...
         .addCase(createReplyThunk.fulfilled, (state, action: PayloadAction<Reply>) => {
            state.creating = false
            const reply = action.payload
            const cId = reply.comment_id
            state.byCommentId[cId] ??= { items: [], meta: null, loading: false, error: null }
            state.byCommentId[cId].items.push(reply)
            if (state.byCommentId[cId].meta) state.byCommentId[cId].meta!.total += 1
         })
      ...
      builder
         ...
         .addCase(updateReplyThunk.fulfilled, (state, action: PayloadAction<Reply>) => {
      ...
      builder
         ...
         .addCase(deleteReplyThunk.fulfilled, (state, action: PayloadAction<{ reply_id: number }>) => {
