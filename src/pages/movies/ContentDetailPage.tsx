...
import { fetchRatingSummary, upsertMyRating as upsertMyRatingThunk, deleteMyRating as deleteMyRatingThunk, selectRatingBucket } from '@/features/rating/ratingSlice'
...
const ContentDetailPage: React.FC = () => {
   ...
   const myCommentItem = useMemo(() => {
      ...
      const found = commentsState.items.find((comment) => String(comment.user_id) === myUserId || String(comment.User?.user_id) === myUserId)

      return found || null
   }, [commentsState.items, user?.user_id])
   ...
   const persistRating = async (uiRating: number | undefined) => {
      if (!topicId || !user?.user_id) return
      if (uiRating == null) return

      const point = Math.max(1, Math.min(10, Math.round(uiRating * 2)))
      try {
         const result = await dispatch(upsertMyRatingThunk({ contentId: topicId, point })).unwrap()
   ...
   }, [isDragging, calculateRating, myRating])
   ...
   const handleCommentDelete = async () => {
      ...
      try {
         const result = await dispatch(deleteCommentThunk({ comment_id: myCommentItem.comment_id })).unwrap()
   ...
   const handleCommentSubmit = async (data: { content: string; isSpoiler: boolean }) => {
      ...
      try {
         if (myCommentItem) {
            ...
            const newComment = await dispatch(createCommentThunk({ topic_id: topicId, content: data.content })).unwrap()
   ...
   const avg5 = ratingBucket?.avg ? ratingBucket.avg / 2 : 0
   ...
   return (
      <>
               ...
               <div className="col-12 col-md-7 col-lg-8 mt-5">
                     ...
                     <div className="ratescore">
                        <div className="rating">
                           ...
                           {ratingBucket?.myPoint != null && (
                              <button className="btn btn-sm btn-outline-secondary mt-2" onClick={deleteMyRating}>
                                 별점 삭제
                              </button>
                           )}
                        </div>
                        ...
                        <div className="score">
                           <div className="score--user">
                              <h4>{myRating !== undefined ? myRating.toFixed(1) : '-'}</h4>
                              <p>나의 별점</p>
                           </div>
                  ...
                  {myCommentItem && (
                     <div className="detail--mycomment-desk mt-5">
                        <div>
                           <p>나의 코멘트</p>
                           {myCommentItem.created_at && <span>{formatDateDot(new Date(myCommentItem.created_at))}</span>}
                        </div>
            ...
            <div className="row movie-comment mt-6">
               <div className="col-12">
                  <div className="row movie-comment__header">
                     ...
                     <Icon icon="mdi:chevron-right" style={{ fontSize: 'icon-md', cursor: 'pointer' }} onClick={() => navigate(`${PATHS.comments}?contentId=${id}`)} />
                  </div>
                     ...
                     {commentsState.loading ? (
                        ...
                        <>
                           {displayedComments.map((c) => (
                              <div key={c.comment_id} className="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
                                 <ImageCommentCard username={getCommentUsername(c)} comment={c.content} rating={5} likes={c.likes ?? 0} replies={c.replies ?? 0} commentItem={c} onClick={handleCommentCardClick} />
                              </div>
                           ))}
         ...
         <CommentEditModal isOpen={showCommentEdit} onClose={closeCommentEdit} onSubmit={handleCommentSubmit} initialContent={myCommentItem?.content} mode={myCommentItem ? 'edit' : 'create'} />
      </>
   )
}
