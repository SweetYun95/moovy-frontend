// moovy-frontend/src/components/Home/MoveCard/MovieCard.tsx
// 외부 라이브러리
...
export const MovieCard: React.FC<MovieCardProps> = ({ id, title, synopsis, imageUrl, className = '', rating, onCommentClick }) => {
   const navigate = useNavigate()
   const handleCommentClick = createContentDetailHandler(navigate, id, onCommentClick)

   return (
      <div className={`movie-card ${className} ${!imageUrl ? 'movie-card--no-image' : ''}`} style={imageUrl ? { backgroundImage: `url(${imageUrl})` } : {}}>
         ...
         <div className="movie-card__content">
            ...
            <div className="movie-card__action">
               <WideButton buttonType="comment" onClick={handleCommentClick} />
            </div>
         </div>
      </div>
   )
