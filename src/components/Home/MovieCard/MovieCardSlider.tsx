// moovy-frontend/src/components/Home/MoveCard/MovieCardSlider.tsx
// 외부 라이브러리
...
export const MovieCardSlider: React.FC<MovieCardSliderProps> = ({ className = '', onCardClick, onSectionsChange }) => {
      ...
      return (
         <div className="movie-card-section mb-4">
            ...
            <div className="movie-card-slider__inner-wrapper">
               <button id={prevId} className="movie-card-slider__nav-button movie-card-slider__nav-button--prev" aria-label="이전 슬라이드">
                  ...
                  {section.movies.map((movie: any) => (
                     <SwiperSlide key={movie.id}>
                        <div className="movie-card-section__grid">
                           <MovieCard id={movie.id} title={movie.title} synopsis={movie.synopsis} imageUrl={movie.imageUrl} rating={movie.overallRating} onCommentClick={onCardClick ? () => onCardClick?.(movie.title) : undefined} />
                        </div>
                     </SwiperSlide>
                  ))}
               </Swiper>

               <button id={nextId} className="movie-card-slider__nav-button movie-card-slider__nav-button--next" aria-label="다음 슬라이드">
   ...
   return (
      <div className={`movie-card-slider ${className}`}>
         <div className="movie-card-slider__wrapper">
            {!isMobile && (
               <button id={prevBtnId} ref={prevButtonRef} className="movie-card-slider__nav-button movie-card-slider__nav-button--prev" aria-label="이전 슬라이드">
                  <Icon icon="mdi:chevron-left" width="24" height="24" />
               </button>
            )}
            ...
            {isMobile ? (
               ...
               <>
                     ...
                     {sections.map((section, index) => (
                        <SwiperSlide key={`section-${index}`}>
                           <div className="movie-card-section">
                              ...
                              <div className="movie-card-section__grid">
                                 {section.movies.map((movie: any) => (
                                    <MovieCard key={movie.id} id={movie.id} title={movie.title} synopsis={movie.synopsis} imageUrl={movie.imageUrl} rating={movie.overallRating} onCommentClick={onCardClick ? () => onCardClick?.(movie.title) : undefined} />
                                 ))}
                              </div>
                           </div>
                        </SwiperSlide>
                  ...
                  <button id={nextBtnId} ref={nextButtonRef} className="movie-card-slider__nav-button movie-card-slider__nav-button--next" aria-label="다음 슬라이드">
                     <Icon icon="mdi:chevron-right" width="24" height="24" />
                  </button>
               </>
            )}
