...
const SanctionHistoryModal: React.FC<SanctionHistoryModalProps> = ({
  isOpen,
  onClose,
  histories,
  onDetailClick,
        ...
        <div className="row">
          <div className="col-12">
            <div className="sanction-history-modal__list">
              {histories.length === 0 ? (
                <div className="sanction-history-modal__empty">
                  경고 기록이 없습니다.
                </div>
              ) : (
                histories.map((history, index) => (
                  <div key={history.id} className="sanction-history-modal__item">
                    <div className="sanction-history-modal__header">
                      제제 {index + 1}회
                    </div>
                    <div className="sanction-history-modal__body">
                      <span className="sanction-history-modal__reason">
                        {history.reason}
                      </span>
                      {onDetailClick && (
                        <span 
                          className="sanction-history-modal__detail-link"
                          onClick={() => onDetailClick(history.id)}
                        >
                          상세신고내역
                        </span>
                      )}
