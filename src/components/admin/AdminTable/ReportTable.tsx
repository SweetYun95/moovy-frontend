...
/**
 * Table.tsx에서 사용하는 행 데이터 타입
 */
export interface AdminReportRow extends AdminReportItem {
   reporter_id: string
   reported_id: string
   ...
   return (
      <>
         {tableData.map((data) => {
            ...
            return (
               <ul className="data" key={data.report_id} onClick={() => onRowClick?.(displayData)} style={{ cursor: 'pointer' }}>
                  <li>
                  ...
                  <li className={`status status--${state === '처리완료' ? 'completed' : 'pending'} ${state === '확인하기' ? 'status--clickable' : ''}`} onClick={(e) => onStatusClick?.(e, displayData)} style={{ cursor: 'pointer' }}>
                     {state}
                  </li>
               </ul>
            )
