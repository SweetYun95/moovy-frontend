// moovy-frontend/src/components/profile/CalendarSection.tsx
...
export const CalendarSection: React.FC = () => {
      ...
      favorites.forEach((favorite) => {
         if (favorite.favorited_at) {
            ...
            map.get(dateKey)!.push({
               ...
               imageUrl: favorite.poster || content?.imageUrl || content?.images?.[0] || null,
   ...
   const getDaysInMonth = (date: Date): CalendarDay[] => {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
         ...
         days.push({
            ...
            contentImage: firstContent?.imageUrl ?? undefined,
   ...
   const handleDayClick = (day: CalendarDay) => {
      if (day.date) {
         if (selectedDate && selectedDate.getFullYear() === day.date.getFullYear() && selectedDate.getMonth() === day.date.getMonth() && selectedDate.getDate() === day.date.getDate()) {
   ...
   const isDateSelected = (day: CalendarDay) => {
      ...
      return day.date.getFullYear() === selectedDate.getFullYear() && day.date.getMonth() === selectedDate.getMonth() && day.date.getDate() === selectedDate.getDate()
   }
         ...
         <div className="col-12 col-md-6">
            <div className="calendar-wrapper">
               ...
               <div className="calendar-header">
                  <button type="button" className="calendar-nav-btn" onClick={goToPreviousMonth} aria-label="이전 달">
               ...
               <div className="calendar-grid">
                  {calendarDays.map((day, index) => (
                     <div key={index} className={`calendar-day ${day.date ? 'has-date' : 'empty'} ${isDateSelected(day) ? 'active' : ''}`} onClick={() => handleDayClick(day)}>
                        {day.date && (
                           <>
                              ...
                              {day.contentImage && (
                                 <div className="calendar-day-content">
                                    <img src={day.contentImage} alt={day.contentTitle} className="calendar-day-poster" />
                                    {day.badgeCount && day.badgeCount > 0 && <span className="calendar-day-badge">{day.badgeCount}</span>}
                                 </div>
                              )}
                           </>
                        )}
         ...
         <div className="col-12 col-md-6">
            <div className="content-posters-wrapper">
                        ...
                        {selectedDateBookmarks.length > 0 ? (
                           <>
                              {displayedBookmarks.map((content) => {
                                 ...
                                 return (
                                    <div key={content.id} className="content-poster-item" onClick={() => handleContentClick(content.id)}>
                                       {content.imageUrl && <img src={content.imageUrl} alt={content.title} className="content-poster-image" />}
                              ...
                              {/* 빈 플레이스홀더 */}
                              {Array.from({
                                 length: Math.max(0, 9 - displayedBookmarks.length),
                              }).map((_, index) => (
               ...
               {selectedDate && selectedDateBookmarks.length > 9 && (
                  <>
                     <button className="content-posters-nav content-posters-nav--prev" onClick={handlePreviousPage} disabled={currentPage === 0} aria-label="이전 페이지">
                        <Icon icon="mdi:chevron-left" width="24" height="24" />
                     </button>

                     <button className="content-posters-nav content-posters-nav--next" onClick={handleNextPage} disabled={currentPage >= totalPages - 1} aria-label="다음 페이지">
                        <Icon icon="mdi:chevron-right" width="24" height="24" />
                     </button>
                  </>
               )}
